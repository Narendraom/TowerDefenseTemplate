...
namespace TowerDefense.UI.HUD
{
	...
	public class GameUI : Singleton<GameUI>
	{
		...
		public void PlaceDefaultMachineGun()
		{
			Tower towerToBuild = LevelManager.instance.towerLibrary[0];

			SingleTowerPlacementArea[] placementAreas = FindObjectsOfType<SingleTowerPlacementArea>();
			if (placementAreas != null && placementAreas.Length > 0)
			{
				m_CurrentArea = placementAreas[UnityEngine.Random.Range(0, placementAreas.Length)];
				m_GridPosition = m_CurrentArea.WorldToGrid(m_CurrentArea.transform.position, towerToBuild.dimensions);
			}
			else
			{
				TowerPlacementGrid[] gridarea = FindObjectsOfType<TowerPlacementGrid>();
				if (gridarea != null && gridarea.Length > 0)
				{
					int index = UnityEngine.Random.Range(0, gridarea.Length);
					m_CurrentArea = gridarea[index];

					int x = UnityEngine.Random.Range(1, gridarea[index].dimensions.x-1);
					int y = UnityEngine.Random.Range(1, gridarea[index].dimensions.y-1);
					m_GridPosition = m_CurrentArea.WorldToGrid(gridarea[index].m_Tiles[x, y].transform.position, towerToBuild.dimensions);
			...
			Tower createdTower = Instantiate(towerToBuild);
			createdTower.Initialize(m_CurrentArea, m_GridPosition);
		}                                   
