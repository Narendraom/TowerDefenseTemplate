...
namespace Core.Health
{
	...
	public class DeathEffect : MonoBehaviour
	{
		...
		void OnDied(HealthChangeInfo healthChangeInfo)
		{
			...
			if(Hovertank!=null)
			{
				GameObject hovertank = Poolable.TryGetPoolable(Hovertank);
				var agentInstance = hovertank.GetComponent<Agent>();
				agentInstance.transform.position = transform.position;
				agentInstance.Initialize();
				agentInstance.SetNode(GetComponent<Agent>().CurrentNode);
				agentInstance.transform.rotation = GetComponent<Agent>().CurrentNode.transform.rotation;
			}
		}
	}
}
